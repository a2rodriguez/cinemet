...
<html>
<head>
<title>Recherche par acteur</title>
...
//<!-- VERSION PDO DEFINTIF -->
  ...
  //La fiche du film 
        ...
        $skl = "SELECT * FROM acteurs WHERE id_act=".$_POST['i_acteur'];
        foreach ($conn->query($skl) as $rows) {
            ?><div class="titre"><?php echo $rows['designation']?></div><?php
        }
        $safidy = $rows['designation'];

        /*$sql = $conn -> query("SELECT * FROM films WHERE acteur='".$safidy."'");*/
        $sql = $conn -> prepare("SELECT * FROM films WHERE acteur=:safy");
