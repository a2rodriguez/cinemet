...
<html>
<head>
<title>Recherche par realisateur</title>
...
//<!-- VERSION PDO DEFINTIF -->
  ...
  //La fiche du film 
        ...
        $skl = "SELECT * FROM realisateurs WHERE id_real=".$_POST['i_real'];
        foreach ($conn->query($skl) as $rows) {
            ?><div class="titre"><?php echo $rows['nom']?></div><?php
        }
        $safidy = $rows['nom'];
        
        //echo "Réalisateur sélectionné : ".$safidy;

        //$sql = $conn -> query("SELECT * FROM films WHERE realisateur='".$safidy."'");
        

        $sql = $conn -> prepare("SELECT * FROM films WHERE realisateur=:safy");
